...
export default (hocOptions) => (PageComponent) => {
  const dataProp = hocOptions.dataProp || 'data';
  ...
  const selectorObject = { [dataProp]: generateSelectApi(hocOptions.storeName) };
  ...
  const saga = generateSaga(hocOptions.storeName);
  const withSaga = injectSaga({
    key: hocOptions.storeName,
    saga,
    mode: DAEMON,
  });
